# ===============================================
# ❓ Module Quản lý Câu hỏi - manage_questions.py (Đã sửa lỗi Loại/Mức độ)
# ===============================================
...
def render(chu_de_options, chu_de_id_list):
    ...
    # --- ĐỊNH NGHĨA CÁC LỰA CHỌN CỐ ĐỊNH (ĐÃ SỬA) ---
    # LOẠI CÂU HỎI (QUYẾT ĐỊNH WIDGET)
    ...
    with tab_add:
        ...
        if not chu_de_options:
            st.warning("⚠️ Chưa có Chủ đề nào trong hệ thống.")
            st.stop()
        else:
            selected_chu_de_name = st.selectbox(
                "**1. Chọn Chủ đề (Bắt buộc)***:",
                list(chu_de_options.keys()),
                key="q_add_cd_select_main",
                index=None,
                placeholder="Chọn chủ đề của câu hỏi..."
            )
        ...
        if selected_chu_de_id:
            with st.form("add_question_form", clear_on_submit=True):
                st.markdown("**3. Nhập nội dung câu hỏi**:")

                # ---- SỬA LỖI Ở ĐÂY ----
                loai = st.selectbox(
                    "Loại câu hỏi * (Cách trả lời):",
                    LOAI_CAU_HOI_OPTIONS,  # Sử dụng list đúng
                    key="q_loai",
                    index=0,
                    help="Quyết định cách học sinh trả lời: 'mot_lua_chon' (chọn 1), 'nhieu_lua_chon' (chọn nhiều), 'dien_khuyet' (điền chữ)."
                )
                muc_do = st.selectbox(
                    "Mức độ * (Độ khó):",
                    MUC_DO_OPTIONS,  # Sử dụng list đúng
                    key="q_muc_do",
                    ...
                    if not noi_dung:
                        ...
                        try:
                            insert_payload = {
                                ...
                                "loai_cau_hoi": loai,  # Giá trị đúng
                                "noi_dung": noi_dung,
                                "dap_an_dung": dap_an_dung,
                                "dap_an_khac": dap_an_khac,
                                "muc_do": muc_do,  # Giá trị đúng
                                "diem_so": diem_so
                            }
            ...
            st.info("Vui lòng chọn Chủ đề để bắt đầu nhập câu hỏi.")

    # --- Tab Danh sách & Sửa/Xóa ---
    with tab_list:
        df_quiz_original = crud_utils.load_data(table_name)
        if not df_quiz_original.empty:
            chu_de_id_map_quiz = {id_: name for name, id_ in chu_de_options.items()}
            bai_hoc_id_map_quiz = {id_: details["name"] for id_, details in bai_hoc_details.items()}

            df_quiz_display = df_quiz_original.copy()
            df_quiz_display['chu_de_id'] = df_quiz_display['chu_de_id'].astype(str).map(chu_de_id_map_quiz).fillna(
                "N/A")
            df_quiz_display['bai_hoc_id'] = df_quiz_display['bai_hoc_id'].astype(str).map(bai_hoc_id_map_quiz).fillna(
                "(Chung)")

            df_quiz_display = df_quiz_display.rename(
                columns={"chu_de_id": "Chủ đề", "bai_hoc_id": "Bài học", "loai_cau_hoi": "Loại", "muc_do": "Mức độ"})
            df_quiz_display = df_quiz_display.sort_values(by=["Chủ đề", "Bài học", "id"]).reset_index(drop=True)

            try:
                df_quiz_display['dap_an_dung_display'] = df_quiz_display['dap_an_dung'].apply(
                    lambda x: ', '.join(map(str, x)) if isinstance(x, list) else x)
                df_quiz_display['dap_an_khac_display'] = df_quiz_display['dap_an_khac'].apply(
                    lambda x: ', '.join(map(str, x)) if isinstance(x, list) and x else '')
            except Exception as e:
                st.warning(f"Lỗi khi định dạng cột đáp án: {e}")
                df_quiz_display['dap_an_dung_display'] = ''
                df_quiz_display['dap_an_khac_display'] = ''

            cols_display_q = ['id', 'noi_dung', 'Chủ đề', 'Bài học', 'Loại', 'dap_an_dung_display',
                              'dap_an_khac_display', 'Mức độ', 'diem_so']
            cols_exist = [col for col in cols_display_q if col in df_quiz_display.columns]

            st.info("Nhấp vào một hàng trong bảng dưới đây để Sửa hoặc Xóa.")
            gb = st.dataframe(df_quiz_display[cols_exist], key="quiz_df_select", hide_index=True,
            ...
            if selected_rows:
                original_id = df_quiz_display.iloc[selected_rows[0]]['id']
                    ...
                    with st.form("edit_question_form"):
                        ...
                        if selected_chu_de_id_edit:
                            filtered_lesson_options_edit = {
                                details["name"]: bh_id
                                for bh_id, details in bai_hoc_details.items()
                                if details["chu_de_id"] == selected_chu_de_id_edit
                            }
                        ...
                        current_bh_name = bai_hoc_id_map_quiz.get(current_bh_id,
                        ...
                        if selected_lesson_id_edit == "NONE_VALUE":
                            selected_lesson_id_edit = None
    ...
    with tab_import_q:
        ...
        sample_data_q = {
            ...
            'loai_cau_hoi': ['mot_lua_chon'],  # Loại widget
            'noi_dung': ['1+1=?'],
            'dap_an_dung': ['2'],
            'dap_an_khac': ['1;3;4'],
            'muc_do': ['biết'],  # Độ khó
                    ...
                    with st.spinner("Đang import câu hỏi..."):
                        for index, row in df_upload.iterrows():
                            try:
                                ...
                                if bai_hoc_id_str and bai_hoc_details[bai_hoc_id_str]['chu_de_id'] != chu_de_id_str:
                                    raise ValueError(
