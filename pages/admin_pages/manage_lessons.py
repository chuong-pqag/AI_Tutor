# ===============================================
# 📝 Module Quản lý Bài học - manage_lessons.py (Đã thêm lọc)
# ===============================================
...
def render(mon_hoc_options, chu_de_df, chu_de_options):
    """
    Hiển thị giao diện quản lý Bài học.
    Args:
        mon_hoc_options (dict): Map {tên môn: id}
        chu_de_df (pd.DataFrame): DataFrame của bảng chu_de
        chu_de_options (dict): Map {tên chủ đề display: id}
    """
    ...
    with tab_add:
        ...
        if selected_mon_hoc_name:
            # Lọc chu_de_options (map {tên display: id})
            filtered_chu_de_options_map = {
                display_name: id
                for display_name, id in chu_de_options.items()
                # Kiểm tra xem 'mon_hoc' của chủ đề có khớp không
    ...
    with tab_list:
        ...
        if not chu_de_df.empty:
            # Dùng merge để lấy thông tin từ bảng chu_de
        ...
        with col_f2:
            ...
            if selected_khoi != "Tất cả":
                mon_hoc_list.extend(sorted(
                    list(df_lesson_display[df_lesson_display['Khối'] == selected_khoi]['Môn học'].dropna().unique())))
            else:
                mon_hoc_list.extend(sorted(list(df_lesson_display['Môn học'].dropna().unique())))
            selected_mon_hoc = st.selectbox("Lọc theo Môn học:", mon_hoc_list, key="lesson_filter_monhoc", index=0)

        with col_f3:
            # Lọc Chủ đề (dựa trên Môn học)
            chu_de_list = ["Tất cả"]
            df_filtered_by_mon = df_lesson_display
            if selected_khoi != "Tất cả":
                df_filtered_by_mon = df_filtered_by_mon[df_filtered_by_mon['Khối'] == selected_khoi]
        ...
        # 3. Lọc DataFrame
        df_to_show = df_lesson_display.copy()
        if selected_khoi != "Tất cả":
            df_to_show = df_to_show[df_to_show['Khối'] == selected_khoi]
        if selected_mon_hoc != "Tất cả":
            df_to_show = df_to_show[df_to_show['Môn học'] == selected_mon_hoc]
        ...
        if not df_to_show.empty:
            ...
            if 'lesson_selected_item_id' in st.session_state:
                ...
                original_item_df = df_lesson_original[df_lesson_original['id'] == selected_id]  # Tìm trong df gốc
                if not original_item_df.empty:
                    selected_item_original = original_item_df.iloc[0].to_dict()

            # 4. Form Sửa/Xóa (Giữ nguyên logic)
            if selected_item_original:
                with st.expander("📝 Sửa/Xóa Bài học", expanded=True):
                    with st.form("edit_lesson_form"):
                        ...
                        # Dùng chu_de_options đã truyền vào
                        ...
                        chu_de_ten_edit = st.selectbox("Thuộc Chủ đề", list(chu_de_opts_local.keys()), index=cd_idx)
                        ...
                        delete_pdf_flag = st.checkbox("Xóa PDF hiện tại", key="del_pdf_flag")
                        ...
                        if col_update.form_submit_button("💾 Lưu thay đổi", use_container_width=True):
                            update_data = {"ten_bai_hoc": ten_bai_hoc_edit,
                                           "chu_de_id": chu_de_opts_local.get(chu_de_ten_edit), "thu_tu": thu_tu_edit,
    ...
    with tab_import:
        ...
        if uploaded_lesson:
            try:
                ...
                valid_chu_de_ids = list(
                    chu_de_options.values()) if chu_de_options else []  # Dùng chu_de_options đã truyền vào
                if not valid_chu_de_ids:
                    st.error("Chưa có chủ đề nào.")
                        ...
                        for index, row in df_upload_lesson.iterrows():
                            try:
                                ...
                                if chu_de_id not in valid_chu_de_ids: raise ValueError(
                                    f"Chu de ID '{chu_de_id}' không hợp lệ.")
