# ===============================================
# 🧑‍🏫 Module Quản lý Phân công Giảng dạy - manage_assignments.py (Đã thêm lọc và sửa lỗi)
# ===============================================
...
def render():
    ...
    with tab_add:
        with st.form("add_assignment_form", clear_on_submit=True):
            ...
            if not gv_options or not lop_options or not mh_options:
                ...
                st.form_submit_button("Thêm phân công", disabled=True)
            else:
    ...
    with tab_list:
        ...
        # Tạo map id -> tên (dùng để hiển thị)
        ...
        if not df_to_show.empty:
            ...
            # 5. Form Sửa/Xóa (ĐÃ SỬA LỖI CHÍNH TẢ)
            if selected_item_original:
                with st.expander("📝 Sửa/Xóa Phân công đã chọn", expanded=True):
                    with st.form("edit_assign_form"):
                        ...
                        # Sử dụng map để hiển thị Tên thay vì ID
                        ...
                        if col_update.form_submit_button("💾 Lưu thay đổi", use_container_width=True):
                            update_data = {"vai_tro": vai_tro_edit, "nam_hoc": nam_hoc_edit}
                            try:
            ...
            # Sửa thông báo
            if df_assign_original.empty:
                st.info("Chưa có phân công giảng dạy nào.")
            else:
                st.info("Không tìm thấy phân công nào phù hợp với bộ lọc.")
