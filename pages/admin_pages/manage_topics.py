# ===============================================
# 📚 Module Quản lý Chủ đề - manage_topics.py (ĐÃ TÁI CẤU TRÚC DATA LOADING)
# ===============================================
...
# Hàm render không nhận các DataFrame/options toàn cục nữa
def render():
    ...
    st.caption(f"Đang quản lý Chủ đề liên quan đến Năm học: **{selected_year}**")

    # 1. Tải Môn học (Master Data - không lọc theo năm)
    mon_hoc_df = crud_utils.load_data("mon_hoc")
    mon_hoc_options = {row["ten_mon"]: str(row["id"]) for _, row in
                       mon_hoc_df.iterrows()} if not mon_hoc_df.empty else {}

    # 2. Lấy Khối (Grades) đang hoạt động trong năm đã chọn
    ...
    # 3. Lọc Chủ đề (Master Data - lọc theo Khối đang hoạt động)
    df_cd_original_all = crud_utils.load_data(table_name)
    df_cd_original = df_cd_original_all[df_cd_original_all['lop'].isin(active_khoi_list)].copy()

    # 4. Tái tạo options chỉ từ dữ liệu đã lọc (ACTIVE)
    chu_de_options_active = {
        f"{row['ten_chu_de']} (L{row['lop']}-T{row['tuan']})": str(row['id'])
        for _, row in df_cd_original.iterrows()
    }
    chu_de_options_with_none_active = {"Không có": None}
    chu_de_options_with_none_active.update(chu_de_options_active)
    # --------------------------------------------

    MUC_DO_OPTIONS = ["biết", "hiểu", "vận dụng"]

    if df_cd_original.empty and not df_cd_original_all.empty:
        st.warning(f"Không tìm thấy Chủ đề nào thuộc Khối lớp đang hoạt động trong Năm học: **{selected_year}**.")

    # --- Tab Thêm mới ---
    with tab_add:
        # ... (logic tab_add giữ nguyên, sử dụng các biến đã tự tải: mon_hoc_options, chu_de_options_with_none_active) ...
        # Lấy danh sách Khối để tạo
        khoi_add_options = sorted([int(k) for k in active_khoi_list])
        ...
        with st.form("add_chu_de_form", clear_on_submit=True):
            ...
            if not khoi_add_options:
                st.error(
                    f"⚠️ Không tìm thấy Khối lớp nào đang hoạt động trong Năm học **{selected_year}**. Vui lòng thêm Lớp học trước.")
                khoi = st.number_input("Khối *", min_value=1, max_value=12, value=1, disabled=True)
            else:
                # Chỉ cho phép chọn Khối có lớp đã tạo trong năm này
                khoi = st.selectbox("Khối *", khoi_add_options, index=0)

            tuan = st.number_input("Tuần *", min_value=1, max_value=52, value=1)

            # Prerequisite Options phải dùng Options ĐÃ LỌC
            prereq_ten = st.selectbox("Tiền đề (Tùy chọn)", list(chu_de_options_with_none_active.keys()),
                                      key="topic_add_prereq", index=0) if chu_de_options_with_none_active else None
            ...
            if submitted:
                if not mon_hoc_options or mon_hoc_ten is None:
                    ...
                    try:
                        # Prerequisite ID phải được lấy từ map đã lọc
                        prereq_id = chu_de_options_with_none_active.get(prereq_ten) if prereq_ten else None

                        insert_payload = {
                            "ten_chu_de": ten_chu_de,
                            "mon_hoc_id": mon_hoc_options.get(mon_hoc_ten),
                            "mon_hoc": mon_hoc_ten,
                            "lop": khoi,  # Lấy từ selectbox đã lọc
                            "tuan": tuan,
                            "prerequisite_id": prereq_id,
                            "muc_do": muc_do
                        }
                        supabase.table(table_name).insert(insert_payload).execute()
                        st.success(f"Đã thêm chủ đề cho Khối {khoi} (Năm {selected_year})!")
                        crud_utils.clear_all_cached_data()
                    except Exception as e:
                        st.error(f"Lỗi khi thêm chủ đề: {e}")

    # --- Tab Danh sách & Sửa/Xóa (SỬ DỤNG DỮ LIỆU ĐÃ LỌC) ---
    with tab_list:

        if df_cd_original.empty:
            st.stop()

        if not df_cd_original.empty:

            # 1. TẠO BỘ LỌC (Chỉ áp dụng cho dữ liệu đã lọc theo năm)
            st.markdown("##### 🔍 Lọc danh sách")
            col_filter1, col_filter2 = st.columns(2)

            # Lấy danh sách Khối (từ dữ liệu đã lọc theo năm)
            ...
            # Ánh xạ Tiền đề (Dùng map ACTIVE)
            chu_de_id_map_display = {id_: name for name, id_ in chu_de_options_active.items()}
            df_cd_display['Tiền đề'] = df_cd_display['prerequisite_id'].astype(str).apply(
                lambda x: chu_de_id_map_display.get(x) if pd.notna(
                    x) and x in chu_de_id_map_display.values() else "Không có")
            ...
            # ... (Phần hiển thị bảng và Form Sửa/Xóa giữ nguyên) ...
            cols_order = ['id', 'ten_chu_de', 'Khối', 'Môn học', 'tuan', 'Mức độ', 'Tiền đề']
            cols_display_cd_ordered = [col for col in cols_order if col in df_to_show.columns] + [col for col in
                                                                                                  df_to_show.columns if
            ...
            if 'cd_selected_item_id' in st.session_state:
                ...
                # Tìm trong DF GỐC TẤT CẢ CHỦ ĐỀ
                original_item_df = df_cd_original_all[df_cd_original_all['id'] == selected_id]
            ...
            if selected_item_original:
                ...
                # Kiểm tra chủ đề này có thuộc Khối đang hoạt động không
                is_active_topic = selected_item_original.get('lop') in active_khoi_list
                disabled_editing = not is_active_topic

                if not is_active_topic:
                    st.warning(
                        f"Chủ đề này thuộc Khối {selected_item_original.get('lop')}, Khối này không hoạt động trong Năm học **{selected_year}**.")

                with st.expander("📝 Sửa/Xóa Chủ đề đã chọn", expanded=True):
                    with st.form("edit_cd_form"):
                        st.text(f"ID: {selected_item_original['id']}")

                        # Dùng map ACTIVE cho các selectbox
                        chu_de_opts_none_local = chu_de_options_with_none_active
                        ...
                        mh_idx = list(mon_hoc_options.keys()).index(
                            current_mh_name) if current_mh_name in mon_hoc_options else 0;
                        mon_hoc_ten_edit = st.selectbox("Môn học", list(mon_hoc_options.keys()), index=mh_idx,
                                                        disabled=disabled_editing)

                        # Khối/Tuần: Lấy từ list đã lọc
                        if not disabled_editing:
                            khoi_edit_options = sorted([int(k) for k in active_khoi_list])
                            khoi_idx_edit = khoi_edit_options.index(
                                selected_item_original.get("lop", 1)) if selected_item_original.get("lop",
                                                                                                    1) in khoi_edit_options else 0
                            lop_edit = st.selectbox("Khối", khoi_edit_options, index=khoi_idx_edit)
                        else:
                            # Nếu bị disable, hiển thị giá trị hiện tại
                        ...
                        if col_update.form_submit_button("💾 Lưu thay đổi", use_container_width=True, disabled=disabled_editing):
                            ...
                            update_data = {
                                ...
                                "mon_hoc_id": mon_hoc_options.get(mon_hoc_ten_edit),
    ...
    with tab_import_cd:
        ...
        st.warning(f"Việc import sẽ áp dụng cho các Khối lớp đang hoạt động trong Năm học: **{selected_year}**")
        ...
        if uploaded_cd:
            try:
                ...
                # Lấy tất cả ID chủ đề có sẵn (bao gồm cả các năm khác) cho validation import
                valid_chu_de_ids_prereq = [""] + [str(row['id']) for _, row in df_cd_original_all.iterrows()]
                ...
                if not valid_mon_hoc_ids:
                    ...
                    active_khoi_set = set(active_khoi_list)  # Khối đang hoạt động (cho validation)
                    ...
                    with st.spinner("Đang import..."):
                        for index, row in df_upload_cd.iterrows():
                            try:
                                ...
                                lop_val = pd.to_numeric(row['lop'], errors='coerce');
                                ...
                                if pd.isna(lop_val) or not (1 <= lop_val <= 12): raise ValueError("Khối không hợp lệ.")

                                lop = int(lop_val)

                                # === VALIDATION KHỐI THEO NĂM HỌC ===
                                if lop not in active_khoi_set:
                                    raise ValueError(f"Khối {lop} không hoạt động trong Năm học **{selected_year}**.")
                                # =====================================
                                ...
                                insert_data = {"ten_chu_de": ten_chu_de, "mon_hoc_id": mon_hoc_id, "lop": lop,
